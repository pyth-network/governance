...
import { PYTH_MINT, WSOL_MINT } from '@components/constants'
...
class TokenService {
  ...
  async fetchSolanaTokenList() {
    try {
      ...
      toast.error('Unable to fetch token list')
    }
  }
  async fetchTokenPrices(mintAddresses: string[]) {
    if (mintAddresses.length) {
      const mintAddressesWithSol = [...mintAddresses, WSOL_MINT, PYTH_MINT]
