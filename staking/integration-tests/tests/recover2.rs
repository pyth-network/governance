use {
    anchor_lang::error::{AnchorError, ErrorCode}, integration_tests::{
        assert_anchor_program_error, governance::{
            ...
            helper_functions::initialize_new_stake_account, instructions::{
                create_position, create_stake_account, join_dao_llc, merge_target_positions, recover_account_2, update_token_list_time, update_voter_weight
            }, pda::{
                get_stake_account_metadata_address, get_target_address, get_voter_record_address
            }
        }, utils::clock::advance_n_epochs
    }, litesvm::LiteSVM, solana_cli_output::CliAccount, solana_sdk::{
        account::{
            AccountSharedData,
            WritableAccount,
        }, native_token::LAMPORTS_PER_SOL, pubkey::Pubkey, signature::Keypair, signer::Signer
    }, staking::state::{
        max_voter_weight_record::MAX_VOTER_WEIGHT, positions::{
            TargetWithParameters,
            POSITION_BUFFER_SIZE,
        }, stake_account::StakeAccountMetadataV2, target::TargetMetadata, voter_weight_record::VoterWeightRecord
    }, std::{
        fs::File,
        io::Read,
        str::FromStr,
...
fn test_recover2() {
    ...
    let stake_account_positions = initialize_new_stake_account(&mut svm, &owner, &pyth_token_mint, true, true);

    assert_anchor_program_error!(recover_account_2(&mut svm, &owner, stake_account_positions, new_owner.pubkey()), 
    ErrorCode::ConstraintHasOne,
    0
    );

    recover_account_2(&mut svm, &governance_authority, stake_account_positions, new_owner.pubkey()).unwrap();
    ...
    let stake_account_metadata: StakeAccountMetadataV2 = fetch_account_data(&mut svm, &get_stake_account_metadata_address(stake_account_positions));
    assert_eq!(stake_account_metadata.owner, new_owner.pubkey());

    let voter_record: VoterWeightRecord = fetch_account_data(&mut svm, &get_voter_record_address(stake_account_positions));
